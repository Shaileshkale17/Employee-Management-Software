  ...
  return (
    <div className="flex">
      ...
      <div className="flex-1 min-h-screen p-4 lg:p-6 bg-surface-100">
        <Link to={`/candidates/${id}`} className="text-sm text-gray-500 hover:text-brand-600 font-medium transition-colors inline-block mb-4">
          ← Back to profile
        </Link>
        <h1 className="text-xl font-bold text-gray-900 mb-6">Edit Candidate</h1>

        {loading ? (
          <div className="animate-pulse h-64 bg-gray-100 rounded-2xl" />
        ) : (
          <form onSubmit={handleSubmit} className="bg-white rounded-2xl shadow-card border border-gray-100 p-6 lg:p-8 space-y-5 max-w-2xl">
